...
class TestActionProposer:
    ...
    def test_propose_invalid_view_type(self):
        ...
        with pytest.raises(TypeError) as exc_info:
            proposer.propose("not a ProposalView")

        assert "`view` should be a ProposalView instance" in str(exc_info.value)
    ...
    def test_propose_empty_view_no_proposals(self):
        ...
        # Create minimal ProposalView with no anomaly, low confidence, or trend
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8  # High confidence (> 0.5)
        stats_mock.last_value = 100.0
    ...
    def test_propose_anomaly_detected(self):
        ...
        # Create ProposalView with anomaly
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 150.0  # Some value
        view_mock.stats = stats_mock
    ...
    def test_propose_anomaly_not_detected(self):
        ...
        # Create ProposalView with anomaly but is_anomaly = False
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 100.0
    ...
    def test_propose_low_confidence(self):
        ...
        # Create ProposalView with low confidence
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.3  # Low confidence (< 0.5)
        stats_mock.last_value = 100.0
        view_mock.stats = stats_mock
    ...
    def test_propose_confidence_none(self):
        ...
        # Create ProposalView with None confidence
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = None  # None confidence
        stats_mock.last_value = 100.0
        view_mock.stats = stats_mock
        view_mock.anomaly = None
        view_mock.trend = None
    ...
    def test_propose_confidence_exactly_0_5(self):
        ...
        # Create ProposalView with confidence = 0.5
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.5  # Exactly 0.5
        stats_mock.last_value = 100.0
        view_mock.stats = stats_mock
        view_mock.anomaly = None
        view_mock.trend = None
    ...
    def test_propose_trend_upward(self):
        ...
        # Create ProposalView with upward trend
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 100.0
    ...
    def test_propose_trend_downward(self):
        ...
        # Create ProposalView with downward trend
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 100.0
    ...
    def test_propose_trend_stable(self):
        ...
        # Create ProposalView with stable trend
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 100.0
    ...
    def test_propose_all_three_conditions(self):
        ...
        # Create ProposalView that triggers all three proposal types
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.3  # Low confidence
        stats_mock.last_value = 150.0
        view_mock.stats = stats_mock
    ...
    def test_propose_trend_none_confidence(self):
        ...
        # Create ProposalView with trend but None confidence
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 100.0
    ...
    def test_propose_anomaly_none_confidence(self):
        ...
        # Create ProposalView with anomaly but None confidence
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = 150.0
    ...
    def test_propose_stats_none_last_value(self):
        """Test propose handles stats with None last_value."""
        proposer = ActionProposer()

        # Create ProposalView with None last_value
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.8
        stats_mock.last_value = None  # None last_value
        view_mock.stats = stats_mock
        ...
        proposal = result[0]
        assert proposal.action == "investigate_anomaly"
        # metadata["last_value"] would be None
    ...
    def test_propose_realistic_sensor_scenario(self):
        ...
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.4  # Low confidence
        stats_mock.last_value = 85.5  # High value
        view_mock.stats = stats_mock
    ...
    def test_propose_server_monitoring_scenario(self):
        ...
        view_mock = create_autospec(ProposalView)
        stats_mock = create_autospec(HistoryStatistics)
        stats_mock.confidence.return_value = 0.95  # High confidence
        stats_mock.last_value = 98.7  # Very high CPU
        view_mock.stats = stats_mock
...
def test_imports():
    ...
    from procela.core.reasoning.view import ProposalView
