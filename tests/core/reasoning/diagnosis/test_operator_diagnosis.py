...
class TestTrendOperator:
    ...
    def test_abstract_method_exists(self):
        ...
        assert annotations["stats"] == "HistoryStatistics"
...
class TestTrendOperatorThreshold:
    ...
    def mock_history_statistics_mean(self, mock_history_statistics):
        ...
        mock_history_statistics.mean.return_value = 48.0
    ...
    def test_analyze_with_none_count(self):
        """Test analyze when stats.count is None."""
        stats = Mock(spec=HistoryStatistics)
        stats.count = None
        stats.ewma = 50.0

        operator = TrendOperatorThreshold()
        result = operator.analyze(stats)
        assert result is None
    ...
    def test_analyze_with_none_mean(self, mock_history_statistics):
        ...
        mock_history_statistics.mean.return_value = None

        operator = TrendOperatorThreshold()
        result = operator.analyze(mock_history_statistics)
        assert result is None
    ...
    def test_analyze_stable_trend(self):
        ...
        stats.count = 10
        stats.ewma = 50.0
        stats.mean.return_value = 48.5  # delta = 1.5, threshold = 3.0
    ...
    def test_analyze_upward_trend(self):
        ...
        stats.count = 10
        stats.ewma = 55.0
        stats.mean.return_value = 48.0  # delta = 7.0, threshold = 3.0

        operator = TrendOperatorThreshold(threshold=3.0)
        result = operator.analyze(stats)

        assert result is not None
        assert result.value == 7.0  # 55.0 - 48.0
    ...
    def test_analyze_downward_trend(self):
        ...
        stats.count = 10
        stats.ewma = 42.0
        stats.mean.return_value = 50.0  # delta = -8.0, threshold = 3.0

        operator = TrendOperatorThreshold(threshold=3.0)
        result = operator.analyze(stats)

        assert result is not None
        assert result.value == -8.0  # 42.0 - 50.0
    ...
    def test_analyze_exact_at_threshold(self):
        ...
        stats.count = 10
        stats.ewma = 53.0
        stats.mean.return_value = 50.0  # delta = 3.0, threshold = 3.0
    ...
    def test_analyze_zero_threshold(self):
        ...
        stats.count = 10
        stats.ewma = 50.1
        stats.mean.return_value = 50.0  # delta = 0.1
    ...
    def test_analyze_with_realistic_values(self):
        ...
        stats.count = 100
        stats.ewma = 42.75
        stats.mean.return_value = 45.20  # delta = -2.45
...
class TestDiagnosisOperator:
    ...
    def test_abstract_method_exists(self):
        ...
        assert annotations["view"] == "DiagnosisView"
...
class TestDiagnosisOperatorThreshold:
    ...
    def mock_diagnosis_view(self):
        """Create a mock DiagnosisView."""
        return Mock(spec=DiagnosisView)
    ...
    def test_diagnose_with_concrete_view(self):
        ...
        # Create a realistic DiagnosisView mock
    ...
    def test_type_annotations_preserved(self):
        ...
        assert diagnose_sig.parameters["view"].annotation == "DiagnosisView"
...
class TestIntegration:
    ...
    def test_trend_operator_full_workflow(self):
        ...
        stats.count = 50
        stats.ewma = 123.45
        stats.mean.return_value = 120.0  # delta = 3.45
        ...
        result2 = operator2.analyze(stats)
...
def test_coverage_edge_cases():
    ...
    stats.count = 10
    stats.ewma = 50.0
    stats.mean.return_value = None
    assert operator.analyze(stats) is None

    # Test with stats that has ewma as 0.0
    stats2 = Mock(spec=HistoryStatistics)
    stats2.count = 10
    stats2.ewma = 0.0
    stats2.mean.return_value = 0.0
    result = operator.analyze(stats2)
    assert result is not None
    assert result.value == 0.0
    assert result.direction == "stable"

    # Test with very large values
    stats3 = Mock(spec=HistoryStatistics)
    stats3.count = 1000
    stats3.ewma = 1_000_000.0
    stats3.mean.return_value = 999_999.5
    result = operator.analyze(stats3)
    assert result is not None
...
def test_type_error_message_format():
    ...
    # Test with non-HistoryStatistics object
    with pytest.raises(TypeError) as exc_info:
        operator.analyze({"not": "stats"})

    error_msg = str(exc_info.value)
    # Check the exact format from the code
    assert "stats should be a HistoryStatistics instance" in error_msg
