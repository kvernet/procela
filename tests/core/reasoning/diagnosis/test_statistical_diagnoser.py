...
class TestStatisticalDiagnoserDiagnoseMethod:
    ...
    def test_diagnose_with_high_variability(self) -> None:
        ...
        # Create mock stats with high coefficient of variation
        mock_stats = Mock()
        mock_stats.count = 50
        mock_stats.mean = lambda: 10.0
        mock_stats.std = lambda: 5.0  # CV = 0.5 > 0.3 threshold
        mock_stats.min = 2.0
        mock_stats.max = 25.0
    ...
    def test_diagnose_with_zero_mean(self) -> None:
        ...
        mock_stats = Mock()
        mock_stats.count = 50
        mock_stats.mean = lambda: 0.0
        mock_stats.std = lambda: 15.0  # High absolute std
        mock_stats.min = -20.0
        mock_stats.max = 25.0
    ...
    def test_diagnose_with_significant_drift(self) -> None:
        ...
        mock_stats.mean = lambda: 100.0
        mock_stats.std = lambda: 8.0
    ...
    def test_diagnose_with_unusual_statistics(self) -> None:
        ...
        mock_stats.count = 50
        mock_stats.mean = lambda: 100.0
        mock_stats.std = lambda: 80.0  # High absolute std
        mock_stats.min = 10.0
    ...
    def test_diagnose_with_trend_z_score_formatting(self) -> None:
        ...
        mock_stats.count = 100
        mock_stats.mean = lambda: 50.0
        mock_stats.std = lambda: 0.08  # z = 0.07/0.08 = 0.875
        mock_stats.min = mock_stats.max = None
    ...
    def test_diagnose_multiple_pattern_confidence_boost(self) -> None:
        ...
        mock_stats.count = 50
        mock_stats.mean = lambda: 10.0
        mock_stats.std = lambda: 2.0  # CV = 0.2 > 0.1 threshold
        mock_stats.min = 5.0
        mock_stats.max = 15.0
...
class TestStatisticalDiagnoserInternalMethods:
    ...
    def test_analyze_variability(self) -> None:
        ...
        mock_stats.count = 50
        mock_stats.mean = lambda: 10.0
        mock_stats.std = lambda: 5.0  # CV = 0.5 > 0.3
        mock_stats.min = 2.0
        mock_stats.max = 25.0
        ...
        mock_stats_normal.mean = lambda: 100.0
        mock_stats_normal.std = lambda: 10.0  # CV = 0.1 < 0.3
    ...
    def test_detect_drift(self) -> None:
        ...
        mock_stats_no_std.std = lambda: None
    ...
    def test_analyze_variability_with_zero_mean_edge_case(self) -> None:
        ...
        mock_stats = Mock()
        mock_stats.count = 50
        mock_stats.mean = lambda: 0.0  # Exactly zero
        mock_stats.std = lambda: 15.0  # High std > 10
        mock_stats.min = -20.0
        mock_stats.max = 25.0
    ...
    def test_detect_drift_with_downward_direction(self) -> None:
        ...
        mock_stats = Mock()
        mock_stats.std = lambda: 0.05  # z = 0.08/0.05 = 1.6 > 1.0
    ...
    def test_detect_drift_with_zero_std(self) -> None:
        ...
        mock_stats = Mock()
        mock_stats.std = lambda: 0.0  # Zero std - should avoid division
    ...
    def test_detect_drift_minor_z_score(self) -> None:
        ...
        mock_trend = Mock()
        mock_trend.value = 0.12  # > sensitivity
        mock_trend.direction = "up"

        mock_stats = Mock()
        mock_stats.std = lambda: 0.15  # z = 0.12/0.15 = 0.8 <= 1.0

        causes = diagnoser._detect_drift(mock_trend, mock_stats)
        assert len(causes) > 0
        assert "minor" in causes[0].lower()  # Should say "minor"
        assert "z=" not in causes[0]  # Minor drift doesn't show z-score
