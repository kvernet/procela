...
def mock_view_with_trend(mock_trend_up):
    """Provides a mock DiagnosisView with a trend."""
...
def mock_view_without_trend():
    """Provides a mock DiagnosisView without a trend."""
    view = Mock(spec=DiagnosisView)
    view.trend = None
    view.stats = None
    view.anomaly = None
...
def mock_view_with_stats_and_anomaly(mock_trend_down):
    """Provides a mock DiagnosisView with stats and anomaly."""
    view = Mock(spec=DiagnosisView)
...
class TestDiagnoseMethod:
    ...
    def test_diagnose_with_invalid_view_type(self, default_diagnoser):
        ...
        assert "must be DiagnosisView" in str(excinfo.value)
...
class TestPrivateMethods:
    ...
    def test_statistical_context(self):
        ...
        class View:
            trend = TrendResult(value=1000, direction="stable", threshold=1000)
            stats = HistoryStatistics(count=20, sum=18, sumsq=200)
            anomaly = None
        ...
        view.stats = HistoryStatistics(count=2, sum=18, sumsq=1700000)
        result = diagnoser._statistical_context(view.trend, view.stats)
        assert len(result) == 1

        view.stats = HistoryStatistics(count=2, sum=18, sumsq=1700, min=1.8)
