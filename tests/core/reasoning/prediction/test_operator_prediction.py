...
class TestPredictionOperator:
    ...
    def mock_prediction_view(self):
        """Create a mock PredictionView object."""
        return Mock(spec=PredictionView)
    ...
    def test_view_type_check_fails_with_wrong_type(self, mock_predictor):
        """Test that view type check fails with non-PredictionView."""
        operator = PredictionOperator()

        with pytest.raises(TypeError) as exc_info:
            operator.predict(mock_predictor, "not a view")

        assert "view should be a PredictionView" in str(exc_info.value)
        assert "got" in str(exc_info.value)
    ...
    def test_type_error_message_format(self):
        ...
        with pytest.raises(TypeError) as exc_info:
            operator.predict(42, Mock(spec=PredictionView))
        ...
        error_msg = str(exc_info.value)
        assert error_msg.startswith("view should be a PredictionView")
        assert "got" in error_msg
    ...
    def test_predict_method_signature(self):
        ...
        assert annotations["view"] == "PredictionView"
...
class TestIntegration:
    ...
    def test_complete_prediction_workflow(self):
        ...
        mock_view = Mock(spec=PredictionView)
    ...
    def test_multiple_predictions_with_same_operator(self):
        ...
        test_cases = [
            (Mock(spec=Predictor), Mock(spec=PredictionView), 1, 100.0),
            (Mock(spec=Predictor), Mock(spec=PredictionView), 5, 105.5),
            (Mock(spec=Predictor), Mock(spec=PredictionView), None, 98.7),
        ]
...
def test_type_check_implementation_details():
    ...
    with pytest.raises(TypeError) as exc_info:
        operator.predict(fake_predictor, Mock(spec=PredictionView))
    ...
    with patch("procela.core.reasoning.prediction.base.Predictor"):
        ...
        result = operator.predict(real_predictor, Mock(spec=PredictionView))
