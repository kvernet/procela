...
from procela.core.memory import HistoryStatistics, VariableRecord
from procela.symbols.key import Key


class TestHistoryStatistics:
    """Test suite for HistoryStatistics class."""
    ...
    def test_update_with_numeric_value(self):
        ...
        initial_stats = HistoryStatistics.empty()
    ...
    def test_update_with_non_numeric_value(self):
        """Test update method with non-numeric value (should return self unchanged)."""
        ...
        initial_stats = HistoryStatistics(
            count=3,
            sum=15.0,
            sumsq=85.0,
            min=4.0,
        ...
        # Update should return self unchanged for non-numeric values
        updated_stats = initial_stats.update(mock_record)
        ...
        assert updated_stats.confidence_sum == 2.4
    ...
    def test_mean_empty(self):
        ...
        stats2 = HistoryStatistics(count=0, sum=None, sumsq=0.0)
    ...
    def test_std_insufficient_data(self):
        ...
        # Test with count < 2
        stats1 = HistoryStatistics.empty()
        assert stats1.std() is None

        stats2 = HistoryStatistics(count=1, sum=10.0, sumsq=100.0)
        assert stats2.std() is None

        # Test with count >= 2 but mean is None
        stats3 = HistoryStatistics(count=2, sum=None, sumsq=50.0)
    ...
    def test_confidence_empty(self):
        ...
        stats2 = HistoryStatistics(count=0, confidence_sum=None)
        assert stats2.confidence() is None

    def test_immutability(self):
        """Test that HistoryStatistics is immutable (frozen dataclass)."""
    ...
    def test_repr_string(self):
        ...
        assert "HistoryStatistics" in repr_str
    ...
    def test_hashable(self):
        """Test that HistoryStatistics is hashable (frozen dataclass)."""
        stats = HistoryStatistics.empty()
