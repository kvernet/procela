...
def trend_result():
    ...
    def _create_mock_trend(
        ...
        return Trend(value, direction, threshold, confidence_val, zscore_val)

    return _create_mock_trend


@pytest.fixture
def mock_view_with_trend(trend_result):
    """Provides a mock VariableView with trend and stats."""
    ...
    def _create_mock_view(
        ...
        stats = StatisticsResult(value=last_value)

        view = Mock(spec=VariableView)
        view.key = None
        view.reasoning = None
        view.anomaly = None
        view.trend = trend
...
def mock_view_no_trend():
    """Provides a mock VariableView without trend attribute."""
    stats = StatisticsResult(value=10.0)
...
def mock_view_none_trend():
    """Provides a mock VariableView with trend = None."""
    stats = StatisticsResult(value=10.0)
...
@pytest.fixture
def mock_view_no_last_value(trend_result):
    """Provides a mock VariableView with last_value = None."""
    trend = trend_result()
...
class TestPredictMethod:
    ...
    def test_predict_upward_trend(self, default_predictor, mock_view_with_trend):
        ...
        assert abs(result.value[1] - 10.5) < 0.001
        assert abs(result.value[2] - 10.75) < 0.001
    ...
    def test_predict_downward_trend(self, default_predictor, mock_view_with_trend):
        ...
        assert abs(result.value[0] - 19.85) < 0.001
        assert abs(result.value[1] - 19.7) < 0.001
    ...
    def test_predict_with_extrapolation_factor(self, mock_view_with_trend):
        ...
        assert abs(predictions_low.value[0] - 10.125) < 0.001
    ...
    def test_predict_invalid_view_type(self, default_predictor):
        ...
        assert "must be VariableView" in str(exc_info.value)
    ...
    def test_predict_confidence_out_of_range(self, default_predictor, trend_result):
        """Test predict with confidence values outside [0, 1] range."""
        # Test confidence > 1.0
        trend_high = trend_result(confidence_val=1.5)
        stats = Mock(value=10.0)
        view_high = Mock(spec=VariableView, trend=trend_high, stats=stats)

        predictions_high = default_predictor.predict(view_high, horizon=1)
        assert abs(predictions_high.value[0] - 10.25) < 0.001

        # Test confidence < 0.0
        trend_low = trend_result(confidence_val=-0.5)
        view_low = Mock(spec=VariableView, trend=trend_low, stats=stats)

        predictions_low = default_predictor.predict(view_low, horizon=1)
        assert abs(predictions_low.value[0] - 10.25) < 0.001

    def test_predict_confidence_method_fails(self, default_predictor, trend_result):
        """Test predict when confidence() method fails."""
        trend = trend_result()
        trend.confidence = Mock(side_effect=TypeError("Not callable"))

        stats = Mock(value=10.0)
        view = Mock(spec=VariableView, trend=trend, stats=stats)
...
class TestEdgeCases:
    ...
    def test_predict_with_negative_last_value(
        ...
        assert abs(result.value[0] - (-4.85)) < 0.001
        assert abs(result.value[1] - (-4.7)) < 0.001
