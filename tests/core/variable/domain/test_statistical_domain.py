...
class TestStatisticalDomain:
    ...
    def build_history_statistics(
        ...
        return HistoryStatistics(count, sum, sumsq, min, max, confidence_sum, sources)
    ...
    def test_validate_with_complete_stats_inside_bounds(self):
        ...
        assert domain.validate(95, stats) is True
        assert domain.validate(105, stats) is True
        assert domain.validate(80, stats) is True  # Lower bound inclusive
        assert domain.validate(120, stats) is True  # Upper bound inclusive

    def test_validate_with_complete_stats_outside_bounds(self):
        """Test validation with complete stats, value outside bounds."""
        domain = StatisticalDomain(k=2.0)
        stats = self.build_history_statistics(count=2, sum=200.0, sumsq=20200.0)
        assert domain.validate(79, stats) is False  # Below lower bound
        assert domain.validate(121, stats) is False  # Above upper bound
    ...
    def test_validate_with_incomplete_stats_missing_mean(self):
        ...
        assert domain.validate(50, stats) is True
    ...
    def test_validate_with_different_k_values(self):
        ...
        assert domain1.validate(95, stats) is True
        assert domain1.validate(85, stats) is False

        # k=2.0: bounds 80-120
        domain2 = StatisticalDomain(k=2.0)
        assert domain2.validate(85, stats) is True
        assert domain2.validate(75, stats) is False

        # k=3.0: bounds 70-130
        domain3 = StatisticalDomain(k=3.0)
        assert domain3.validate(75, stats) is True
        assert domain3.validate(65, stats) is False
    ...
    def test_validate_non_numeric_value_raises_error(self):
        ...
        assert not domain.validate("not_a_number", stats)
    ...
    def test_validate_with_zero_std(self):
        ...
        assert domain.validate(50, stats) is True
        assert domain.validate(50.0, stats) is True
        assert domain.validate(51, stats) is False
        assert domain.validate(49, stats) is False
    ...
    def test_explain_with_incomplete_stats(self):
        ...
        explanation = domain.explain(50, stats)
        assert explanation == "Insufficient history for statistical validation."
    ...
    def test_explain_with_different_k_values(self):
        ...
        domain1 = StatisticalDomain(k=1.0)
        exp1 = domain1.explain(95, stats)
        assert exp1 == "Value 95 is within [90.0, 110.0]."

        domain2 = StatisticalDomain(k=2.0)
        exp2 = domain2.explain(95, stats)
        assert exp2 == "Value 95 is within [80.0, 120.0]."

        domain3 = StatisticalDomain(k=3.0)
        exp3 = domain3.explain(95, stats)
    ...
    def test_explain_with_float_precision(self):
        ...
        explanation = domain.explain(52.0, stats)
    ...
    def test_trend_threshold(self):
        ...
        with pytest.raises(ValueError, match="Provide either absolute or std_factor."):
            domain.trend_threshold(stats=stats, absolute=None, std_factor=None)

        res = domain.trend_threshold(stats=stats, absolute=0.58, std_factor=None)
        assert res == 0.58

        res = domain.trend_threshold(stats=stats, absolute=None, std_factor=1.0)
        assert res == 4.0
    ...
    def test_type_annotations(self):
        ...
        assert hints["stats"] == HistoryStatistics | None
    ...
    def test_edge_case_nan_in_stats(self):
        ...
        # NaN in sum
        stats1 = HistoryStatistics(3, float("nan"), 409, None, None, 1.0, frozenset())
        # Comparison with NaN always returns False
        assert domain.validate(50, stats1) is False

        # NaN in sumsq
        stats2 = HistoryStatistics(3, 300, float("nan"), None, None, 1.0, frozenset())
        assert domain.validate(50, stats2) is False
    ...
    def test_edge_case_infinity_in_stats(self):
        ...
        stats1 = HistoryStatistics(
            3, float("inf"), float("inf"), None, None, 1.0, frozenset()
        )
        assert domain.validate(float("inf"), stats1) is False

        # Infinity in sumsq
        stats2 = HistoryStatistics(3, 39, float("inf"), None, None, 1.0, frozenset())
        assert domain.validate(50, stats2) is True
        assert domain.validate(float("inf"), stats2) is True
        assert domain.validate(float("-inf"), stats2) is True
